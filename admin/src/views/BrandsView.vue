...
import { adminApi, type Brand } from '@/api/admin'
...
const load = async () => {
  items.value = (await adminApi.brands()).items
}
...
const toggle = async (item: Brand) => {
  await adminApi.updateBrand(item.id, { is_active: !item.is_active })
  await load()
}
...
<template>
  <main>
    ...
    <form class="inline-form" @submit.prevent="add">
      <input v-model="name" placeholder="新品牌名称" />
